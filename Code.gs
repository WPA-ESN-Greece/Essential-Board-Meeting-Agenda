...
linkCellContents('  📝 Meeting Notes',Meeting.notesUrl,NewAgenda,'A2:B2')
...
//Text Replacing for Notes
 ...
 var body = DocumentApp.openById(notesDoc.getId()).getBody()
  ...
  body.findText('{{📰 Agenda Link}}').getElement().asText().setText('📰 Agenda').setLinkUrl(Meeting.agendaUrl).setFontSize(11).setForegroundColor('#000000').setBold(false).setUnderline(true)
...
var emailToAddresses = ""
  for(var i = 0; i< EVENT_GUESTS.length; i++){

    var emailToAddresses = emailToAddresses + EVENT_GUESTS[i] +","

  }

Logger.log(emailToAddresses)

MailApp.sendEmail(
    {
            to: emailToAddresses,//SLACK_CHANNEL_EMAIL,
            //cc: ,
